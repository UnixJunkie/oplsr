...
(** [(nb_comp_opt, r2) = optimize debug nb_features train_csv_fn nb_folds]
    ...
    [nb_features] is the number of features in each data item.
    [train_csv_fn] is the name of the CSV file holding your training data.
    ...
    [nb_folds] is the number of folds of cross validation used during training;
...
(** [trained_model_fn = train debug nb_features train_csv_fn nb_comp_opt]
    train a model using the given (optimal) number of components [nb_comp_opt]
    and training set CSV file [train_csv_fn].
    Each data item must have [nb_features] number of features.
